...
set libexec_dir=%rel_dir%\libexec\win

set ROOT=%release_root_dir%
set EMU=beam
...
set none_boot_script=%release_root_dir%\bin\start_none
...
:: Get the first argument as the task name, shift the remaining arguments
set taskname="%1"
shift /1
...
:: Escape parens in parameter
set param=%param:(=^^(%
set param=%param:)=^^)%

:: If there is no value for this argument, we're done
if "%~1"=="" goto :args_parsed

:: If we have arguments, append to them, start them otherwise
:: Also, when expanding the argument, escape quotes
if "!largs!"=="" (
  set largs=%param:"=\"%
) else (
...
:: Set space as the argument delimiter
set "delim= "

:: Shift to the next argument and loop
shift /1
goto :parse_args
...
set ERTS_LIB_DIR=%release_root_dir%\lib
goto :eof
...
:pingpeer
call %libexec_dir%\pingpeer.bat %args%
goto :eof
...
setlocal EnableDelayedExpansion
call %libexec_dir%\release_remote_ctl.bat rpc !args!
endlocal
goto :eof

:: Eval an expression locally
:eval
setlocal EnableDelayedExpansion
call %libexec_dir%\release_ctl.bat eval !args!
endlocal
goto :eof
...
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%
setlocal EnableDelayedExpansion
set get_pid_cmd=%libexec_dir%\pid.bat
...
if %pid% equ +%pid% (
  ...
  echo "Node must be down, or pid detection returned non-numeric value"
  exit /b 1
)
exit /b 1
...
call :pre_configure_hooks
call :post_configure_hooks
call %libexec_dir%\release_remote_ctl.bat reload_config --sysconfig="%sys_config%"
goto :eof
